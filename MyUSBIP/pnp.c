...
--*/
{
	...
	status = IoRegisterDeviceInterface(
		...
		NULL,
		&deviceData->InterfaceName); // symbol link name
	...
	Bus_KdPrint(deviceData, BUS_DBG_SS_TRACE,
		("AddDevice: %p to %p->%p (%ws) ,deviceData->InterfaceName:(%ws)\n",
			deviceObject,
			deviceData->NextLowerDriver,
			PhysicalDeviceObject,
			deviceName,
			deviceData->InterfaceName)); // \\Device\\000000b2
...
NTSTATUS
Bus_PnP(
	__in PDEVICE_OBJECT   DeviceObject, //指向Bus_AddDevice 创建的FDO, 或者是虚拟USB设备的PDO
	__in PIRP             Irp
)
/*++
Routine Description:
...
--*/
{
	...
	Bus_KdPrint_Def(BUS_DBG_SS_TRACE, ("Bus_PnP: 0x%p\n",
		DeviceObject));
...
--*/
{
	...
	case IRP_MN_START_DEVICE:
		...
		if (NT_SUCCESS(status)) {
			...
			{
				KdPrint(("Is STATUS_PENDING\n"));
			}
			KdPrint(("Bus_FDO_PnP:IRP_MN_START_DEVICE:Bus_StartFdo\n"));
			//
	...
	--*/
{
	...
	// PnP state : started , stoped , remove....
...
--*/
{
	...
	if (status == STATUS_PENDING) {
		...
		{
			KdPrint(("Bus_SendIrpSynchronously:status == Irp->IoStatus.Status\n"));
		}
		if (Irp->IoStatus.Status == STATUS_MORE_PROCESSING_REQUIRED)
		{
			KdPrint(("Bus_SendIrpSynchronously:Irp->IoStatus.Status == STATUS_MORE_PROCESSING_REQUIRED\n"));
		}
		else if (Irp->IoStatus.Status == STATUS_SUCCESS)
		{
			KdPrint(("Bus_SendIrpSynchronously:Irp->IoStatus.Status == STATUS_SUCCESS\n"));
		}
		else if (Irp->IoStatus.Status == STATUS_PENDING)
		{
			KdPrint(("Bus_SendIrpSynchronously:Irp->IoStatus.Status == STATUS_PENDING\n"));
		}
...
--*/
{
	...
	if (Irp->PendingReturned == TRUE) {
		...
		{
			KdPrint(("IoGetCurrentIrpStackLocation((Irp))->Control & SL_PENDING_RETURNED\n"));
		}
		// 这里为什么不需要 IoMarkIrpPending();
